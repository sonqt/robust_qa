...
def main():
    ...
    def prepare_train_features(examples):
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            start_labels = [0] * len(sequence_ids)
            end_labels = [0] * len(sequence_ids)
            ...
            if len(answers["answer_start"]) == 0:
                tokenized_examples["start_positions"].append(start_labels)
                tokenized_examples["end_positions"].append(end_labels)
            else:
                for ans_id in range(len(answers["answer_start"])):
                    # Start/end character index of the answer in the text.
                    start_char = answers["answer_start"][ans_id]
                    end_char = start_char + len(answers["text"][ans_id])
                    ...
                    if not (offsets[token_start_index][0] <= start_char and offsets[token_end_index][1] >= end_char):
                        ...
                        while token_start_index < len(offsets) and offsets[token_start_index][0] <= start_char:
                            token_start_index += 1
                        start_labels[token_start_index - 1] = 1
                        while offsets[token_end_index][1] >= end_char:
                            token_end_index -= 1
                        end_labels[token_end_index + 1] = 1
                tokenized_examples["start_positions"].append(start_labels)
                tokenized_examples["end_positions"].append(end_labels)
